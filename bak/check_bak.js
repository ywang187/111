...
function getCart() {
    ...
    if (cart != null) {
		...
		if(money>0 && money<75.00){
			productHtml += '<tr class="cart_item">';
            productHtml += ' <td class="product-name">';
            productHtml += 'Shipping <strong class="product-quantity"> × 1</strong>';
            productHtml += '</td>';
            productHtml += '<td class="product-total">';
            productHtml += ' <span class="amount">$10</span>';
            productHtml += '</td>';
            productHtml += ' </tr>';
			money=money+10;
		}
...
$('#check').on('click', function () {
    ...
    //判断提交选项是否正确
        ...
        judgeFrom("firstName_dif") && judgeFrom("lastName_dif") && judgeFrom("streetAddress_dif")  && judgeFrom("city_dif") && judgeFrom("state_dif") && judgeFrom("phone_dif") && judgeFrom("email_dif") && judgeFrom("referral")) {
